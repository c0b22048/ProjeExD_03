...
class Beam:
    ...
    def __init__(self,bird:Bird):
        ...
        self._rct.left = bird._rct.right  #rectに座標を設定,こうかとんのちょっと右
...
def main():
    ...
    while True:
        ...
        if bomb is not None:
            bomb.update(screen)
            if bird._rct.colliderect(bomb._rct):    #爆弾が存在しているとき
        ...
        bird.update(key_lst, screen)#bird,bomは常にある 

        if beam is not None: #beamはスペース押したときだけだからif文使ってる、beam = noneはwhile文の前で定義
            beam.update(screen)
